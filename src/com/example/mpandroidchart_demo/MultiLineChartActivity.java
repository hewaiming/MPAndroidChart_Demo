...
public class MultiLineChartActivity extends Activity {
	...
	private void init_chart() {
		...
		/*
		 * xVals = new ArrayList<>(); yVals = new ArrayList<>(); random = new
		 * Random(); for (int i = 0; i < 12; i++) { float profix =
		 * random.nextFloat(); yVals.add(new Entry(profix, i)); xVals.add((i +
		 * 1) + "月"); }
		 * 
		 * dataSet = new LineDataSet(yVals, "公司年度利润");
		 * dataSet.setColors(ColorTemplate.COLORFUL_COLORS); data = new
		 * LineData(xVals, dataSet);
		 * 
